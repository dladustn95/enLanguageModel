...
class FeedforawardAdapter(nn.Module):
    def __init__(self, hidden_size=64, input_size=768, init_scale=1e-3):
        ...
        nn.init.normal_(self.up_proj.bias,std=init_scale)
        self.act = gelu_new
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.keyword_module = config.keyword_module

        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True, keyword_module=self.keyword_module, use_adapter=self.adapter) for _ in range(config.n_layer)])
