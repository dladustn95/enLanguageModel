...
class Block(nn.Module):
    ...
    def forward(self, query, key, value, key_key=None, key_value=None, attention_mask=None, head_mask=None):
        attn_outputs = self.attn(query, attention_mask=attention_mask, head_mask=head_mask)
        a = attn_outputs[0]
        if self.use_adapter:
            a = self.adapter1(a)
        x = query + a
        ...
        output_attn = self.multihead_attn(
            x, key, value, attention_mask=attention_mask, head_mask=head_mask
        )
        a = output_attn[0]  # output_attn: a, present, (attentions)
        x = x + a
        x = self.ln_3(x)
        if self.keyword_Module == "attention":
            output_attn = self.multihead_attn2(x, key_key, key_value, attention_mask=attention_mask, head_mask=head_mask)
            a = output_attn[0]  # output_attn: a, present, (attentions)
            x = x + a
            x = self.ln_4(x)
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    Examples::
        ...
        for i, block in enumerate(self.h):
            ...
            outputs = block(hidden_states, bert_hidden, bert_hidden, key_key=attention_score, key_value=attention_score, attention_mask=attention_mask, head_mask=head_mask[i])
